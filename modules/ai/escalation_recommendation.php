...
$result = recommendEscalation($ticket);
logAIInteraction(getCurrentUserId(), $ticketId, 'Escalation Recommendation', 'Escalation check for ticket #' . $ticket['ticket_number'], json_encode($result['data'] ?? ''), $result['success'] ? 'Success' : 'Failed', $result['error'] ?? null);
